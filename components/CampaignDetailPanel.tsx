      ...
      <div className="flex-1 overflow-auto p-5 sm:p-7">
        ...
        {detail && !loading && <>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            <div className="rounded border border-rule bg-white p-4"><p className="text-[10px] uppercase tracking-wide text-ink-faint">Best day · <AcronymText>{stats.role === "sales" ? "ROAS" : stats.role === "tofu" ? "CPM" : "Cost / landing view"}</AcronymText></p><p className="mt-2 text-sm font-semibold">{stats.best ? displayDate(stats.best.date) : "—"}</p><p className="mt-1 text-xs text-moss">{stats.best ? stats.role === "sales" ? `${stats.best.score.toFixed(2)}×` : money(stats.best.score) : "No qualifying day"}</p></div>
            <div className="rounded border border-rule bg-white p-4"><p className="text-[10px] uppercase tracking-wide text-ink-faint">Worst day · <AcronymText>{stats.role === "sales" ? "ROAS" : stats.role === "tofu" ? "CPM" : "Cost / landing view"}</AcronymText></p><p className="mt-2 text-sm font-semibold">{stats.worst ? displayDate(stats.worst.date) : "—"}</p><p className="mt-1 text-xs text-brick">{stats.worst ? Number.isFinite(stats.worst.score) ? stats.role === "sales" ? `${stats.worst.score.toFixed(2)}×` : money(stats.worst.score) : "No result" : "No spend"}</p></div>
